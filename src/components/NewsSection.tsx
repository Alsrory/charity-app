...
export default function newssSection() {
  const [currentPage, setCurrentPage] = useState(1)
  const [newss, setnewss] = useState<news[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const safenewss = Array.isArray(newss) ? newss : []
  const totalPages = Math.ceil(safenewss.length / ITEMS_PER_PAGE)
  const startIndex = (currentPage - 1) * ITEMS_PER_PAGE
  const endIndex = startIndex + ITEMS_PER_PAGE
  const currentnewss = safenewss.slice(startIndex, endIndex)
  ...
  const fetchnewss = async () => {
    try {
      ...
      const response = await api.get('/comments')  // Send GET request to the server
      const newss = response.data               // Extract the response data
     
      console.log('API Response:', response.data)
      // Check if 'data' key exists and is an array
      if (newss && Array.isArray(newss.data)) {
        setnewss(newss.data)                // Set the newss state with the array
      } else {
        setnewss([])                           // If not an array, set an empty array
      }
  
    } catch (error) {
      setError('An error occurred while loading the newss')  // Handle errors
      setnewss([])                                            // Clear news list on error
    } finally {
  ...
  useEffect(() => {
    fetchnewss()   // Fetch newss when the component mounts
  }, [])
  ...
  const handlePageChange = (page: number) => { // Handle page change
    ...
    const element = document.getElementById('newss') // Get the newss element
    if (element) {
  ...
  if (loading) {
    return (
      <section id="newss" className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold mb-4">اخر الأخبار</h2>
            <div className="h-1 w-24 bg-primary mx-auto"></div>
            <p className="text-gray-600 mt-4 max-w-2xl mx-auto">
           اخر الأخبار المتعلقة بالجمعية ومايتصل بهامن مواضيع مهمة 
            </p>
          </div>
    ...
    <section id="newss" className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {currentnewss.map((news: news) => (
            <div key={news.id} className="bg-white rounded-lg shadow-lg overflow-hidden group hover:shadow-xl transition-shadow duration-300">
              <div className="relative h-48">
              <Image // Show the news image
                src={news.photo_url || '/images/image.png'}
                alt={news.title}
                fill
        ...
        <div className="text-center mt-10"> {/* Show the link to all newss */}
          <Link
            href="/newss"
